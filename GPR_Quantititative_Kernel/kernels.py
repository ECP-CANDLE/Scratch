...
class Hyperparameter(namedtuple('Hyperparameter',
    ...
    log : bool, default: True
        Whether the value of this hyperparameter will be log-transformed.
        Hyperparameters such as length_scale are naturally log-scale.
        Derivatives are calculated on the transformed values; 
        the transformation is reversed when getting theta values.
    """
    ...
    def __new__(cls, name, value_type, bounds, n_elements=1, fixed=None, 
        ...
        return super(Hyperparameter, cls).__new__(
            cls, name, value_type, bounds, n_elements, fixed, log)
...
class CompoundKernel(Kernel):
    ...
    def get_params(self, deep=True):
        ...
        params : mapping of string to any
            Parameter names mapped to their values.
        """
        params = dict(kernels=self.kernels)
        if deep:
            for i, kernel in enumerate(self.kernels):
                deep_items = kernel.get_params().items()
                params.update(('c{}__{}'.format(i, k), val) for k, val in deep_items)
        return params
    ...
    def set_params(self, **params):
        ...
        for key, value in six.iteritems(params):
            ...
            if len(split) > 1:
                ...
                name, sub_name = split
                if name[:1] == 'c':
                    sub_object = self.kernels[int(name[1:])]
                    sub_object.set_params(**{sub_name: value})
                else:                  
                    if name not in valid_params:
                        raise ValueError('Invalid parameter %s for kernel %s. '
    ...
    def hyperparameters(self):
        ...
        for i, k in enumerate(self.kernels):
            for hyperparameter in k.hyperparameters:
                r.append(Hyperparameter("k{}__{}".format(i, hyperparameter.name),
